...
public class CommentService {
    ...
    @Transactional
    @CacheEvict(value = "commentCounts", key = "#comment.post.id")
    public void deleteComment(Long commentId, String password, String username) {
        ...
        // 관리자가 아니고 게스트 댓글인 경우 비밀번호 확인
        if (!isAdmin && comment.isGuest()) {
            if (password == null || !validateCommentPassword(comment, password)) {
                throw new CustomException(ErrorCode.INVALID_COMMENT_PASSWORD);
            }
        }
        // 관리자가 아니고 로그인 사용자의 댓글인 경우 작성자 확인
        else if (!isAdmin && !comment.isGuest() && 
                (username == null || !username.equals(comment.getAuthor().getUsername()))) {
    ...
    @Transactional
    @CacheEvict(value = "commentCounts", key = "#comment.post.id")
    public void updateComment(Long commentId, CommentRequestDto dto, String password, String username) {
        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new CustomException(ErrorCode.COMMENT_NOT_FOUND));
